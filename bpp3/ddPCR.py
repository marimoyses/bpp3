...
class BpP3obj():   
    ...
    #  For
    # @profile
    def getP3input(self,ID,condition=['Mask', 'paired', 60, 3, 140, 50, 140]):
        ...
        maxS,minS,optsize=condition[4],condition[5],condition[6]
        if returncode!=0:
        ...
        # if self.data[ID]['SV_Type']=="DUP" and self.data[ID]['DIR_A']=="+" and self.data[ID]['DIR_B']=="-": 
            # print("For SVID {} we treat DUP as DEL with normal direction PCR primers")
        # if self.data[ID]['SV_Type']=="DUP" and self.data[ID]['DIR_A']=="-" and self.data[ID]['DIR_B']=="+": 
        ...
        self.data[ID]['p3inputCood']=[self.data[ID]['START_A'],self.data[ID]['START_B']]
        self.data[ID]['p3input']=helper.getsequence(self.data[ID]['CHR_A'],self.data[ID]['START_A'],self.data[ID]['START_B']-self.data[ID]['START_A'])
        # self.data[ID]['llength']=llength
        cmd='sed -i "s/optS/{}/;s/maxS/{}/g" P3tempSettings.temp'.format(str(optsize),str(maxS)) #!!!!
            ...
            while npairs==0 and condition_number<len(helper.P3ranklist):  # Iterate through the conditions until non zero result
                ...
                # if self.data[ID]['SV_Type']=="INV":
                    ...
                    # elif self.data[ID]['DIR_A']=="-" and self.data[ID]['DIR_B']=="-":
                        # if llength>size:
                            # llength=size-1
                            # print('For CNV {} the left length is longer than the SV length, using llength={} instead'.format(ID,str(rlength)))
                    # else:
                        # raise ValueError("SV ID {} is marked as INV but the breakpoint orientation is incorrect.".format(ID))                
                self.getP3input(ID,condition)   # In case of DUPs rlength/llength will be changed
                ...
                if npairs !=0:
                    ...
                    for i in range(0,p3outputobj.npair):
                        primer=helper.primerobj2(p3outputobj[i],self.data[ID]['p3inputCood'],condition,ID,self.data[ID]['CHR_A'],self.data[ID]['CHR_B'])
                        primer.do_isPCR(self.port)
                        if (primer.QC['isPCR']==1):# and self.data['DIR_A']=="+" and self.data[DIR_B]=="-"):
